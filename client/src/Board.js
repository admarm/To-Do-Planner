...
function Board({ userId }) {
        ...
        axios.post('http://localhost:5000/lists', newList)
            .then(res => {
                if (res.data.message === 'List added') {
                    ...
                    setLists(l => [...l, newListNameTrimmed]); // Functional update
    ...
    }, [newListName, boardId, lists]); // Added 'lists' to fix warning
        ...
        axios.put(`http://localhost:5000/lists/${listId}`, { name: newName.trim() })
            .then(res => {
                if (res.data.message === 'List renamed') {
                    ...
                    console.log('Sending payload to /cards/rename-list:', { userId, oldListName: oldName, newListName: newName.trim() });
                    axios.put('http://localhost:5000/cards/rename-list', { userId, oldListName: oldName, newListName: newName.trim() })
                        .then(res => {
                        ...
                        .catch(err => {
                            console.error('Error from /cards/rename-list:', err.response?.data);
    ...
    const SimpleList = React.memo(({ column }) => {
        ...
        }, [newCardTitle, column]); // Removed 'boardId', 'dispatch', 'userId'
